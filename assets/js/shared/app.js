...
function buildTabBar(sidebar) {
  ...
  const current = location.pathname.split('/').pop() || 'actions.html';
  const pages = [['actions.html', 'Today'], ['waiting.html', 'Waiting'], ['team.html', 'People']];
...
const RELATED = [
  ...
  ['https://nurah-skills.github.io/hsg-mailer-management/', 'Mailer board']
];
...
function boardResults(term) {
  ...
  PERSON_NAMES.forEach((person) => {
    if (has(person, COLLEGE_NAMES[COLLEGE_OF[person]])) {
      found.push({
        group: 'Salesperson', label: person, detail: COLLEGE_NAMES[COLLEGE_OF[person]],
        href: `team.html#${encodeURIComponent(person)}`
      });
    }
  });

  FORMS.forEach(([code, name, college, owner, state]) => {
    if (has(code, name, owner)) {
      found.push({
        group: 'Form', label: name,
        detail: `${code} · ${COLLEGE_NAMES[college]} · ${owner || 'nobody recorded'}`,
        href: `forms.html?search=${encodeURIComponent(code)}`
      });
    }
  });
  ...
  FORM_FINDINGS.forEach(([key, label, detail]) => {
    if (has(label, detail)) {
      found.push({ group: 'Finding', label, detail, href: `forms.html#${key}` });
    }
  });

  REPAIRS.forEach((repair) => {
    if (has(repair.id, repair.problem, repair.owner, formName(repair.form))) {
      found.push({
        group: 'Repair', label: formName(repair.form),
        detail: `${repair.id} · ${repair.priority} · ${repair.owner}`,
        href: `repairs.html#${repair.state}`
      });
    }
  });
...
function buildHeaderTools() {
  ...
  refresh.type = 'button';
  refresh.append(icon(ICONS.refresh, 16), document.createTextNode('Refresh the sheets'));
  refresh.addEventListener('click', () => showToast(`Sample figures, so nothing refreshes. The sheets were read at ${SNAPSHOT.leadsRead} and that reading is fixed.`));
...
function buildStateCard(sidebar) {
  ...
  read.id = 'mail-read';
  read.textContent = SNAPSHOT.leadsReadShort;
  read.title = SNAPSHOT.leadsRead;
...
function buildFooter() {
  ...
  main.append(create('p', 'page-foot', 'HSG · SAST · read-only. The source workbooks stay in charge of the work itself.'));
}

function markCurrentPage(sidebar) {
  const here = location.pathname.split('/').pop() || 'actions.html';
